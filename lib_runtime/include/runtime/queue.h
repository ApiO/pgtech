...
namespace pge
{
  namespace queue 
  {
    ...
    template<typename T> void reserve(Queue<T> &q, u32 size);
    ...
    template <typename T> void consume(Queue<T> &q, u32 n);
  ...
  namespace queue_internal
  {
    ...
    template<typename T> void increase_capacity(Queue<T> &q, u32 new_capacity)
    {
      u32 end = array::size(q._data);
      array::resize(q._data, new_capacity);
      if (q._offset + q._size > end) {
        u32 end_items = end - q._offset;
    ...
    template<typename T> void grow(Queue<T> &q, u32 min_capacity = 0)
    {
  ...
  namespace queue 
  {
    template<typename T> inline u32 size(const Queue<T> &q)
    {
      return q._size;
    }

    template<typename T> inline u32 space(const Queue<T> &q)
    {
    ...
    template <typename T> inline void consume(Queue<T> &q, u32 n)
    {
    ...
    template <typename T> void push(Queue<T> &q, const T *items, u32 n)
    {
      ...
      const u32 size = array::size(q._data);
      const u32 insert = (q._offset + q._size) % size;
      u32 to_insert = n;
  ...
  template <typename T> inline T & Queue<T>::operator[](u32 i)
  {
    return _data[(i + _offset) % array::size(_data)];
  }

  template <typename T> inline const T & Queue<T>::operator[](u32 i) const
  {
    return _data[(i + _offset) % array::size(_data)];
  }
}