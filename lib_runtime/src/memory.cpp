...
namespace
{
  ...
  public:
    ...
    ~MallocAllocator() {
      ...
      // destroyed.
      XASSERT(_total_allocated == 0, "_total_allocated = %u", _total_allocated);
    }
  ...
  public:
    ...
    virtual void *allocate(u32 size, u32 align) {
      ...
      if ((int)(size + sizeof(Header)+align - 1) >(_end - _begin)) {
        // Too large for ring buffer, use backing.
  ...
  inline void lock()
  {
    i32 r = mtx_lock(&_memory_globals.mutex);
    ASSERT(r == thrd_success);
  }

  inline void unlock()
  {
    mtx_unlock(&_memory_globals.mutex);
  }
}
...
namespace pge
{
  namespace memory_globals
  {
    ...
    void shutdown() 
    {
      ...
      mtx_destroy(&_memory_globals.mutex);
