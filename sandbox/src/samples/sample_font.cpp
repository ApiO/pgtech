...
namespace app
{
  ...
  namespace sample_font
  {
    ...
    void init()
    {
      ...
      r = glm::angleAxis(glm::radians(-90.f), glm::vec3(0.f, 0.f, 1.f));
      scale = world::spawn_text(global_game_world, global_font_name, "Scale", TEXT_ALIGN_RIGHT, IDENTITY_TRANSLATION, IDENTITY_ROTATION);
      text::set_scale(global_game_world, scale, 4.f);

      p.x *= -1;
      color = world::spawn_text(global_game_world, global_font_name, "Color", TEXT_ALIGN_RIGHT, p, IDENTITY_ROTATION);
